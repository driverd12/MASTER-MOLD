...
def main() -> int:
    ...
    prompt = build_cursor_prompt(agent_id, objective)
    model_output = run_cursor(prompt=prompt, workspace=workspace)
...
def read_payload() -> dict[str, Any]:
    ...
    try:
        ...
        print(f"[cursor_bridge] invalid json payload: {error}", file=sys.stderr)
        return {}
...
def build_cursor_prompt(agent_id: str, objective: str) -> str:
    return (
...
def run_cursor(prompt: str, workspace: Path) -> str:
    cmd = [
        "cursor",
        "agent",
        "--print",
    ...
    try:
        ...
        print(f"[cursor_bridge] cursor agent failed: {error}", file=sys.stderr)
        return ""

    stdout = proc.stdout or ""
    stderr = proc.stderr or ""
    if stderr.strip():
        print(f"[cursor_bridge] stderr: {compact(stderr)}", file=sys.stderr)
    return extract_cursor_result(stdout)
