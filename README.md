...
- `npm run local:training:prepare` + `npm run local:training:train` now form a truthful bounded training lane: prepare curates the packet, train runs an MLX LoRA pass against a trainable companion model, emits adapter artifacts plus `training_metrics.json`, and leaves live Ollama promotion for a later explicit gate.
