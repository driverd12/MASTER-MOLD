...
- Office GUI stutters or a browser reload hangs: run `npm run agents:off && npm run agents:on` to restart the launchd HTTP runner. For GB-scale stores, set `TRICHAT_OFFICE_REFRESH_SECONDS=10`, `TRICHAT_OFFICE_SNAPSHOT_CACHE_MAX_AGE_SECONDS=300`, `TRICHAT_OFFICE_SNAPSHOT_LIVE_THROTTLE_SECONDS=30`, and `TRICHAT_OFFICE_SNAPSHOT_NODE_TIMEOUT_MS=60000`; the live lane remains fresh while the slower detail snapshot is amortized and allowed to finish. A 30-second `ANAMNESIS_INBOX_POLL_INTERVAL` also avoids empty-queue churn. The runner defaults office snapshot refreshes to a separate STDIO child process so cached GUI reads do not block `/health`, `/ready`, or other MCP client traffic. During a live office rally, use `/office/api/action-status` as the truth surface while intake/action work is still running; the GUI pauses its periodic snapshot pulls during that window and resumes snapshot telemetry after the action settles.
...
- Autonomy keepalive is fail-safe disabled by default because a full maintenance tick can monopolize the single HTTP owner. Keep core, Office, inbox, and snapshot services online with `AUTONOMY_KEEPALIVE_ENABLED=0`; opt in with `1` only during a supervised maintenance window. Enabled runs use bounded scheduled retries without spawning a competing STDIO fallback.
...
- Chronicle desktop-context doctor: run `npm run doctor:chronicle` from a Codex desktop session when `desktop.context` or the federation sidecar reports stale or missing screen context; use `npm run doctor:chronicle:json` for machine-readable output. It checks the live recorder pid, current rolling frame directory, latest frame freshness, display count, pid path compatibility, and the next operator action without printing OCR text or screenshot content. The default freshness budget is 5 minutes because Chronicle can update sparsely on idle or multi-display Macs; pass `-- --max-freshness-seconds <n>` for stricter visual work. The probe accepts current `codex_chronicle` pid files, current lock-file pid evidence, and older `codex_tape_recorder` pid locations so remote Macs do not fail open or false-negative during app-version churn.
- Remote peer staging: run `npm run federation:stage-peer -- --ssh <user@host> --host-id <host-id> --remote-peer http://<host>:8787 --local-peer http://<this-host>:8787 --json` when the peer Mac is online but actively being used. It only prints commands. Read-only checks are separated from idle-required actions so you can collect evidence now and defer Codex/Chronicle or launchd restarts until the operator says the peer is safe to interrupt.
- Storage evidence review: run `npm run storage:review -- --json` to summarize storage guard quarantine/recovery evidence without deleting it. Run `npm run storage:review -- --open-scan --json` immediately before any archive/delete action so open files are checked. Archive/delete commands printed by the tool still require separate action-time confirmation.
